...
if __name__ == '__main__':
    ...
    def disc_train_step(input_batch, target_batch, step):
        with tf.GradientTape() as disc_tape:
            ...
            disc_loss_total = disc_loss + 10*gp

        discriminator_gradients = disc_tape.gradient(disc_loss_total, discriminator.trainable_variables)
        discriminator_optimizer.apply_gradients(zip(discriminator_gradients, discriminator.trainable_variables))

        if step % epoch_steps == 0:
            tf.print("Discriminator loss:  ", disc_loss_total)

        with summary_writer.as_default():
            tf.summary.scalar('disc_loss', disc_loss_total, step=step//epoch_steps)
    ...
    def check_on_test_images(generator, test_ds, epoch):
        ...
        for j in images_to_save:
            ...
            plt.savefig(f'{Config.results_path}/test_input_{j}_{epoch}.jpg')
            plt.imshow(display_list[3])
            plt.savefig(f'{Config.results_path}/test_output_{j}_{epoch}.jpg')
    ...
    def check_on_oos_images(generator, epoch):
        ...
        for index, test_dir in enumerate(test_dirs):
            ...
            plt.imshow(scatterer)
            plt.savefig(f'{Config.results_path}/oos_input_{index}_{epoch}.jpg')
            plt.imshow(y_pred[0, :, :])
            plt.savefig(f'{Config.results_path}/oos_output_{index}_{epoch}.jpg')
    ...
    """" Train GAN"""
    def fit_gan(train_ds, test_ds, steps):
        ...
        for step, (input_batch, target_batch) in train_ds.repeat().take(steps).enumerate():
            ...
            train_step(np.float32(input_batch), np.float32(target_batch), step, disc_train)
    ...
    """" Train WGAN """
    def fit_wgan(train_ds, test_ds, steps):
        ...
        gen_train_ds = train_ds.repeat(Config.EPOCHS).as_numpy_iterator()
        disc_train_ds = train_ds.repeat(Config.EPOCHS * Config.N_CRITIC).as_numpy_iterator()
        ...
        for step in tf.range(steps):
            ...
            for i in range(0, Config.N_CRITIC):
                input_batch, target_batch = disc_train_ds.next()
                disc_train_step(np.float32(input_batch), np.float32(target_batch), step)

            input_batch, target_batch = gen_train_ds.next()
            gen_train_step(np.float32(input_batch), np.float32(target_batch), step)
    ...
    if Config.MODEL == "gan":
        fit_gan(train_dataset, test_dataset, steps=total_steps)
    if Config.MODEL == "wgan":
        fit_wgan(train_dataset, test_dataset, steps=total_steps)
